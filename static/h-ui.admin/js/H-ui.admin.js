...
//获取图片路径并回显
function uploader(tt) {
    var srcs = getObjectURL(tt.files[0]); //获取路径
    $(tt).nextAll(".img1").hide(); //this指的是input
    $(tt).nextAll(".img2").show(); //fireBUg查看第二次换图片不起做用
    $(tt).nextAll('.close').show(); //this指的是input
    $(tt).nextAll(".img2").attr("src",srcs); //this指的是input
